...
#include <WiFiClient.h> // Include WiFiClient for HTTP requests

const char* ssid = "";   // wifi ssid
const char* password = "";   // wifi password
const char* serverIP = "192.168.xx.xxx"; // Replace with your Java server IP  -- change needed 
const int serverPort = 8080; // Java server port --- change needed 
...
void sendRelayStateToServer() {
  WiFiClient client;

  if (client.connect(serverIP, serverPort)) {
    String data = "{\"Relay1\": \"" + String(relayState1 ? "ON" : "OFF") + 
                  "\", \"Relay2\": \"" + String(relayState2 ? "ON" : "OFF") +
                  "\", \"Relay3\": \"" + String(relayState3 ? "ON" : "OFF") + "\"}";
...
void handleControl() {
  ...
  // Send the updated relay state to the Java server
  sendRelayStateToServer();
